...
modelPath=/home/amos/MgDB_amos/checkpoints/test
...
while true; do
    ...
    python -m accelerate.commands.launch --num_processes=1  train.py --train $trainFile --valid $validFile \
        --model_type dae --epochs 100 --batch-size 512 \
        --dim_z 32 --save-dir $modelPath \
        --is-ladder --ladder-beta-type uniform --ladder-loss-type type_1 --lambda_adv 0 --lambda_sim 0 --lambda_margin 1 --lambda_kl 1 --lambda_quant 0.0 \
        --fixed-lambda-quant --rescaled-margin-type "quadratic" --similar-noise 0.03 --divergent-noise 0.2 \
        --loss-reduction mean --lr 0.0001 --distance_type cosine \
        --log-interval 10 \
