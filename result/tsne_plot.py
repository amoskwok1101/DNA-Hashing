...
parser.add_argument('--save-file', type=str, default='tsne_cosine_hamming_divergences_grid.png',
                    help='file name of the saved plot')
parser.add_argument('--metrics', type=str, default='cosine,hamming',
                    help='2 metric used for t-SNE choise: (cosine, euclidean, hamming)')
...
def toBinary(code):
    ...
    binary_code = (code > 0.0).astype(int)
    return binary_code
...
for col, divergence in enumerate(divergences):
    ...
    for index, metric in enumerate(metrics):
        ...
        # Perform t-SNE
        tsne_metric = TSNE(n_components=2, metric='precomputed', random_state=42,init='random')
        embeddings_2d = tsne_metric.fit_transform(distances)
        ...
        axes[index, col].set_xlim(x_lim)
        axes[index, col].set_ylim(y_lim)
